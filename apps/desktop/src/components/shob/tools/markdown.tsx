...
export function Markdown(
  ...
  const { text, cacheKey, streaming, ...others } = props
  const marked = useMarked()
  const i18n = useI18n()
  const [root, setRoot] = useState<HTMLDivElement | null>(null)
  const [html, setHtml] = useState(fallback(text))
  const copyCleanupRef = useRef<(() => void) | undefined>(undefined)
  const copyLabelsRef = useRef({ copy: "", copied: "" })
  const parseSeqRef = useRef(0)
  copyLabelsRef.current = { copy: i18n.t("ui.message.copy"), copied: i18n.t("ui.message.copied") }
  ...
  useEffect(() => {
    ...
    const fetchHtml = async () => {
      ...
      if (typeof window === "undefined") {
        if (!cancelled) setHtml(fallback(src.text))
        return
      }
      ...
      try {
        const list = await Promise.all(
          stream(src.text).map(async (block, index) => {
        ...
        if (!cancelled && parseSeqRef.current === seq) {
          setHtml(list.join(""))
        }
      } catch {
        if (!cancelled) setHtml(fallback(src.text))
    ...
    if (streaming) {
      setHtml(fallback(text))
      const raf = requestAnimationFrame(() => { fetchHtml() })
      return () => {
        cancelled = true
  ...
  }, [text, cacheKey, marked, streaming])
