      ...
      <div class="mx-auto flex min-h-full w-full max-w-5xl items-start px-8 py-14 sm:px-12 lg:px-16">
        <div class="grid w-full gap-12 lg:grid-cols-[minmax(0,300px)_minmax(0,1fr)]">
          <section class="space-y-10">
            <div class="space-y-2">
              <h1 class="text-3xl font-light uppercase tracking-[0.18em] text-white sm:text-4xl">Terminal Workspace</h1>
            </div>

            <div class="space-y-4">
              <h2 class="text-sm font-medium uppercase tracking-[0.24em] text-zinc-500">Start</h2>
              <div class="space-y-2">
                <button
                  type="button"
                  onClick={() => void onOpenFolder()}
                  class="flex items-center gap-3 text-left text-[16px] font-light text-zinc-200 transition hover:text-white"
                >
                ...
                <button
                  ...
                  class="flex items-center gap-3 text-left text-[16px] font-light text-zinc-200 transition hover:text-white disabled:cursor-not-allowed disabled:text-zinc-600"
                >
          ...
          <section class="space-y-4">
            <div class="flex items-center gap-3">
              <SquareTerminal class="h-4 w-4 text-zinc-500" stroke-width={1.7} />
              <h2 class="text-sm font-medium uppercase tracking-[0.24em] text-zinc-500">Recent</h2>
            </div>
            {recentProjects().length === 0 ? (
              <p class="text-sm text-zinc-500">No recent folders yet. Open a project folder to get started.</p>
            ) : (
              <div class="space-y-1">
                {recentProjects().map((project) => {
                  ...
                  return (
                    ...
                    >
                      <span class={`h-2 w-2 rounded-full ${isCurrent ? "bg-zinc-100" : "bg-transparent"}`} />
                      <span class="truncate text-[15px] font-normal tracking-[0.01em]">{projectLabel(project)}</span>
                      <span class={`truncate font-mono text-[12px] ${isCurrent ? "text-zinc-400" : "text-zinc-600"}`}>
                        {project.path}
                      </span>
                      <span class={`shrink-0 text-[11px] uppercase tracking-[0.18em] ${isCurrent ? "text-zinc-300" : "text-zinc-600"}`}>
