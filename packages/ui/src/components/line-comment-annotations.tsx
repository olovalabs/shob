...
type LineCommentStateProps<T> = {
  ...
  commenting: SelectedLineRange | null
...
type LineCommentControllerProps<T extends LineCommentShape> = {
  ...
  renderCommentActions?: (comment: T, controls: { edit: VoidFunction; remove: VoidFunction }) => React.ReactNode
  editSubmitLabel?: string
  onDraftPopoverFocusOut?: React.FocusEventHandler<HTMLDivElement>
  getHoverSelectedRange?: SelectedLineRange | null
...
function AnnotationView<T>(props: {
  current: LineCommentAnnotationMeta<T>
  renderComment: (comment: T) => CommentProps
  renderDraft: (range: SelectedLineRange) => DraftProps
}): React.ReactNode {
  const { current, renderComment, renderDraft } = props

  if (current.kind === "comment") {
    const view = renderComment(current.comment)
    if (view.editor) {
      return (
        <LineCommentEditor
          inline
  ...
  const view = renderDraft(current.range)
  return (
    <LineCommentEditor
      inline
      value={view.value}
...
export function createLineCommentAnnotationRenderer<T>(props: {
  ...
  const mount = (meta: LineCommentAnnotationMeta<T>) => {
    ...
    const node = {
      ...
      setMeta: (meta: LineCommentAnnotationMeta<T>) => {
        currentMeta = meta
        renderContent()
      },
    }
...
export function createLineCommentState<T>(props: LineCommentStateProps<T>) {
  ...
  const setDraft = (value: string) => setState((prev) => ({ ...prev, draft: value }))
  const setEditing = (value: T | null) => setState((prev) => ({ ...prev, editing: value }))
  ...
  return {
    ...
    isEditing: (id: T) => state.editing === id,
    closeComment,
    openComment,
    toggleComment,
    openDraft,
...
export function createLineCommentController<T extends LineCommentShape>(
  ...
  annotations: DiffLineAnnotation<LineCommentAnnotationMeta<T>>[]
...
): DiffLineAnnotation<LineCommentAnnotationMeta<T>>[]
export function createLineCommentAnnotations<T>(
...
export function createLineCommentHoverRenderer(props: {
  label: string
  getSelectedRange: () => SelectedLineRange | null
